...
- **Ship 3 feature target REQ-002 locked:** `cairn spec validate` response envelope extension (`specs_scanned: {requirements, tasks}`) + new `cairn spec init` command that scaffolds annotated `.yaml.example` templates. Absorbs the silent-on-empty-tree and missing-scaffold items from Ship 2 canary polish. Renamed-template detection: `specs/*/REQ-001.yaml` containing the template marker comment fires validation error `kind: renamed_template`.
